...
function renderQuestions() {
    ...
    for (let j = 0; j < totalAnswers.length; j++) {
        ...
        let buttonEl = $("<button>");
        buttonEl.addClass("button medium large-only-expanded answer-button").html(incorrectAnswersRes[j]);
...
function scoreQuiz() {
    ...
    if (score >= 9) {
        $("#scores").append("Well Done! Keep up the good work and go watch some more movies.");
    } else if (score >= 7) {
        $("#scores").append("Your Score wasn't horrible. Improve your chances next time by checking out more movies.");
    } else if (score >= 5) {
        $("#scores").append("Well... might I suggest incorporating some more movie nights?");
    } else if (score >= 2) {
        $("#scores").append("Who's keeping score anyway? Here go check out some movies.");
    } else {
        $("#scores").append("Your feelings matter. You should treat yourself to more movies.");
    }
}
...
$("#movieSearchBtn").on("click", function (event) {
    ...
    $.ajax({
        ...
        // Years since movie was released
        var releasedSince = $("<p>").text(
            `${moment().format("YYYY") - response.Year} year(s) since this movie came out!`
        );
        movieDiv.append(releasedSince);
